...
export default function LogisticaCalendarioPage({ routes, onEditRoute, onCreateRoute }) {
  const { user } = useAuth();
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className={`group min-h-[90px] rounded-xl p-2 border transition-all ${
              isToday ? "bg-primary/10 border-primary/50" : isWeekend ? "bg-red-500/5 border-red-500/20" : dayRoutes.length > 0 ? "bg-surface border-border hover:border-primary/30" : "bg-dark/30 border-border/50 hover:border-primary/30"
            }`}>
              <div className="flex items-center justify-between mb-1">
                <span className={`text-xs font-bold ${isToday ? "text-primary" : isWeekend ? "text-red-400" : "text-muted"}`}>{day}</span>
                {onCreateRoute && ds >= todayStr && (
                  <button
                    onClick={(e) => { e.stopPropagation(); onCreateRoute(ds); }}
              ...
              {dayRoutes.length > 3 && <div className="text-[9px] text-muted text-center">+{dayRoutes.length - 3} más</div>}
            </div>
          );
        })}
      </div>
